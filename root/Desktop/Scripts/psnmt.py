...
import _dbHandler      # output generation
...
if ret == 0:
    ...
    # I'm only interested in Protocol Source IP, Source Port, Destination
    # IP, Destination Port
    ipObservations=[]
    ...
    # Write each unique sorted packet to the database
    for packet in finalList:
        oDB.writeDBRow(packet)
    oLog.writeLog('INFO','Program End')
    
    # Close the Log and DB objects
    del oLog
    del oDB
