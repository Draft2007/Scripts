...
# encrypt some input data by giving it input, a beginning offset, and and 
...
# The salt must be at least 8 bytes in length, with 16 bytes a reasonable max
...
function Get-IntFromBits ([String] $Bits) { [System.Convert]::ToInt32($Bits,2) } 
...
function Get-BitsFromInt ([Int] $Integer, [Switch] $NoLeadingZeros) 
{ 
...
##############################################################################
#
# Vernam's XOR cipher, use it with 2 rounds for doubleplus good security!
#
##############################################################################
