...
#    Must enable the "Audit logon events" audit policy to log this data
#    (or with Advanced Audit Policy, must enable the "Audit Logon" and
#    "Audit Other Logon/Logoff Events" audit policies).
#
...
# Get name of target computer and its local time in DMTF format (it might be many timezones away, so $WithinLastHours is relative to target).
$target = get-wmiobject -computername $computername -query "SELECT LocalDateTime,CSName FROM Win32_OperatingSystem" -authentication packetprivacy -impersonation impersonate 
...
# Convert time to System.DateTime and subtract $WithinLastHours, then convert back to DMTF format again for the WMI query.
...
#Build array of the LogonType code numbers in 4624 events; used for filtering output.
$typesarray = [Int32[]] @()
...
#Default to InteractiveLogons if no logon types are specified, and exclude computer account logons by default.
if ($typesarray.count -eq 0) { foreach ($i in @(2,7,9,10,11)) { $typesarray += $i } } 
if ($IncludeComputerAccountLogons) { $userfilter = 'mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm' } else { $userfilter = '*$' } #Computer accounts end with '$'.    

# Unfortunately, the InsertionStrings array is not filterable with WQL queries, so all filtering must be done locally.
# This is a big limitation of WMI because we'll quickly hit quota limits, but there's no way I know around it.
...
#Now grab the event log data through WMI (rewrite to use get-cim* later).
get-wmiobject -computername $computername -query $query -authentication packetprivacy -impersonation impersonate | 
...
foreach { `
    ...
    {
        #Network Logons
        '3'  { $output.LogonType = 'Network(3)' ; break } 
        '8'  { $output.LogonType = 'NetworkCleartext(8)' ; break }
        #Interactive Logons
        '2'  { $output.LogonType = 'Interactive(2)' ; break }  
        '4'  { $output.LogonType = 'BatchScheduledTask(4)' ; break }              
        '5'  { $output.LogonType = 'ServiceStart(5)' ; break }              
        '7'  { $output.LogonType = 'UnlockDesktop(7)' ; break } 
        '9'  { $output.LogonType = 'NewNetworkCredentials(9)' ; break }  #RUNAS.EXE /NETONLY              
        '10' { $output.LogonType = 'RemoteDesktop(10)' ; break } 
        '11' { $output.LogonType = 'CachedCredentials(11)' ; break } 
        default { $output.LogonType = 'Unknown(' + "$_" + ')' }
    }
    
    #If a property is blank, user might assume there is a problem or bug, so replace blank properties with '<not-logged>'.
...
# Here are the InsertionStrings index numbers for reference, in case you need to extract more:
#
#  0 : SubjectUserSid 
#  1 : SubjectUserName 
#  2 : SubjectDomainName 
