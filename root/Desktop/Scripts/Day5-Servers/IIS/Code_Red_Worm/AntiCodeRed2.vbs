...
'     Version: 1.9
'      Author: Jason Fossen, The SANS Institute
...
'       Notes: This script is part of the 5-day "Securing Windows" series of
...
'              for the tens of thousands of users on the Internet who have been infected.
'***********************************************************************************
...
'                Back up the metabase before any changes are made to it.
...
sPatchInfo = sPatchInfo & "   http://www.microsoft.com/technet/security/bulletin/MS01-044.asp" & vbCrLf
sPatchInfo = sPatchInfo & "   Microsoft Security Bulletin MS01-044" & vbCrLf
sPatchInfo = sPatchInfo & "   Cumulative Patch for IIS" & vbCrLf
sPatchInfo = sPatchInfo & " " & vbCrLf
...
sPatchInfo = sPatchInfo & "Good luck, and best wishes." & vbCrLf
...
'***********************************************************************************
             sUsageInfo = vbCrLf 
sUsageInfo = sUsageInfo &  "AntiCodeRed2.vbs (v.1.9) Written by Jason Fossen, The SANS Institute" & vbCrLf
...
If bDisable Then Call StopAndDisableWWW(sReport, bDisable)
...
'***********************************************************************************
' Back up the metabase to a file named Pre-AnitCodeRed-Script.MD*
'***********************************************************************************
Sub BackupMetabase(ByRef sReport)
    On Error Resume Next
    ...
    sReport = sReport & "Metabase will be backed up before any changes are made. (FYI)" & vbCrLf
    
    Set oIIS = GetObject("IIS://localhost") 
    oIIS.Backup "Pre-AntiCodeRed-Script", MD_BACKUP_NEXT_VERSION, (MD_BACKUP_SAVE_FIRST + MD_BACKUP_FORCE_BACKUP)

    If Err.Number = 0 Then
        sReport = sReport & "Metabase backed up to Pre-AntiCodeRed-Script.MD* file. (Good)" & vbCrLf
    Else
        'This should really only happen if GetObject() fails, in which case CheckVirtualFoldersInMetabase() procedure will catch it.
        'A backup of the metabase is not essential for cleaning the box, and the metabase is not in danger of permanent corruption anyway.
        sReport = sReport & "Metabase was NOT backed up successfully, but script will continue. (FYI)" & vbCrLf   'Notice the (FYI) instead of (Bad).
    End If
...
Sub ShowPatchWebsites()
    ...
    oIE.Navigate("http://www.microsoft.com/technet/security/bulletin/MS01-044.asp")
    oIE.Visible = 1
