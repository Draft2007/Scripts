...
foreach ($lastfile in $files) `
{
    ...
    # Sanity check size of archive file just read in (test with 4-char password and 1024-bit pub key).
    if ($ciphertext.count -lt 300) 
    { 
    ...
    { 
        $output.StatusMessage = "ERROR: Decryption of symmetric key and IV failed, possible archive file corruption." 
        $output.Valid = $false
        $output
        continue 
    }
