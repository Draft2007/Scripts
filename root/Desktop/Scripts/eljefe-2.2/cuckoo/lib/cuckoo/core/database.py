...
class RMachinery(Base):
    """Configured machinery to be used for virtualization"""
    __tablename__ = "remote_machinery"
    ...
    datastore = Column(String(255), nullable=True)
    ...
    def __repr__(self):
        return "<Machinery('{0}')>".format(self.name)
...
class Database(object):
    ...
    def add_remote_machinery(self,
        ...
        session = self.Session()
        try:
            remote_machinery = RMachinery(name=name,
                                   product=product,
                                   ip=ip,
                                   port=port,
                                   user=user,
    ...
    def set_rmachinery_credentials(self, name, user, password):
        """Set remote machinery credentials.
        @param name:  machinery name 
        ...
        session = self.Session()
        try:
            machinery = session.query(RMachinery).get(name)
            machinery.user = user 
            machinery.password = password
            session.commit()
            session.refresh(machinery)
            return machinery
        except SQLAlchemyError as e:
            log.debug("Database error setting machinery credentials: {0}".format(e))
            session.rollback()
        finally:
            session.close()
    ...
    def list_tasks_by_binary(self, binary_sha1,limit=None, category=None, offset=None, status=None, not_status=None):
        """Retrieve list of task of certain binary.
        @param limit: specify a limit of entries.
        @param category: filter by category
        @param offset: list offset
        @param status: filter by task status
        ...
        session = self.Session()
        try: 
            search = session.query(Sample,Task)

            #join with samples
            search = search.filter(Sample.sha1 == binary_sha1)     
                
            search = search.filter(Sample.id == Task.sample_id)     
            """if status:
                search = search.filter(Task.status == status)
            if not_status:
                search = search.filter(Task.status != not_status)
    ...
    def dropall_tasks(self):
        """Drop all information on Task.
        @return: operation status.
        """
        session = self.Session()
        try:
            task = session.query(Task).delete()
            #session.delete(task)
            session.commit()
        except SQLAlchemyError as e:
            log.debug("Database error deleting tasks: {0}".format(e))
            import pprint
            pprint.pprint(format(e))
            session.rollback()
            return False
        finally:
            session.close()
    ...
    def view_remote_machinery(self, name):
        """Show remote machinery.
        @params name: module name 
        @return: remote machinery's details
        """
        session = self.Session()
        try:
            machinery = session.query(RMachinery).filter(RMachinery.name == name).first()
        except SQLAlchemyError as e:
            log.debug("Database error viewing remote machinery: {0}".format(e))
            return None
        else:
            if machinery:
                session.expunge(machinery)
        finally:
            session.close()
        return machinery
