...
if __name__ == '__main__':
    ...
    # Create a process Pool with 2 processing mapping to
    # the 4 cores on my laptop
    coreCount = multiprocessing.cpu_count()
    corePool = multiprocessing.Pool(processes=coreCount)
    
    # Map the corePool to the hashFile function
    results = corePool.map(hashFile, (HASHDIR+listOfFiles[0], HASHDIR+listOfFiles[1], HASHDIR+listOfFiles[2], HASHDIR+listOfFiles[3]))
