...
# Converts the REG_BINARY value to an actual MAC address
def val2addr(val):
    addr = ''
    for ch in val:
        addr += '%02x ' % ord(ch)
...
# Extracts the network name and MAC address for every listed 
# network profile from the specific keys in the Windows registry
# Uses _winreg library
def printNets(username, password):
    ...
    for i in range(100):
        try:
            ...
            print "Unable to Enumerate Registry Keys!"
            break
