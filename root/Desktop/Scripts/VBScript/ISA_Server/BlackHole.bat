...
REM   Author: Jason Fossen (www.ISAscripts.org) 
...
REM           The local machine is presumed to be Microsoft ISA Server, but
REM           the script actually works on all Windows XP and later computers.
...
REM which is on the same subnet as one of the interfaces of the ISA Server.  For
REM example, if the ISA Server has an internal IP address and subnet mask of
